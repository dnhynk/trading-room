...
A, A-2, B, C와 독립 special 매매 전략의 시장 관측, 주문 실행, 재생과 검증을 관리하는 Python 프로젝트다.
...
| [Special — ARX 선물 캠페인](track_special/README.md) | Bitget ARXUSDT 선형 무기한 long-only observe/paper/replay 연구(실거래 차단) |
...
공개 설정의 기본값은 **A/B 정지·dry, C observe, Special ARX observe/live-disabled**다. `config/tracks.json`과 각 트랙의 설정은 배포 예시이며 실제 계좌 상태를 나타내지 않는다. 키·서버 주소·장부·녹화·학습 모델·운영 평가 등록과 비공개 Git 이력은 포함하지 않는다. 라이브 실행에는 별도의 자격 증명, 계정·네트워크 확인, 소스와 일치하는 고정 모델 및 명시적인 운영 설정이 필요하다.
