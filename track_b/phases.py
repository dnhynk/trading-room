"""Phase evidence — does the lifecycle read (track_b/whale.py) actually pay, and what do the recordings' order flow add? (2026-09-03, the
세력대항마 stage-2 table; read-only, a nightly section.)
  python -m track_b.phases SYM[,SYM] [--day YYYYMMDD] [--from "YYYY-MM-DD HH:MM"] [--hours 96] [--files F...]
...
from track_a.cycles import build as build_cycles
from track_b.whale import timeline, load, WHALE
from track_a.replay import lines as replay_lines
...
def end_ms_of(day, end=None):
    ...
    meaning (track_b.whale's timeline prints KST). The old line applied the local zone twice (mktime -> gmtime -> mktime) and in KST
