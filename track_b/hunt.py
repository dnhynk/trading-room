...
lifecycle PHASE (track_b/whale.py).
  python -m common.supervise hunt      (python -m track_b.hunt [--once] [--dry])

The basket selector (track_a/scan.py + track_a/select.py) is untouched and stays the contract; this module inherits its skeleton — scan ->
...
(track_a.select must be stopped; the job refuses to write while logs/select.pid names a LIVE select job — `pid_alive`, a pid number alone
...
Footprints and thresholds: track_b/whale.py (stage 1: candles + ticker; stage 2: CVD / OI / funding series and the fingerprint tables).
...
from track_a.scan import PRODUCT
from common.signal import wilder_atr
from track_b.whale import footprints, phase as whale_phase, longer_history, pre_qualifies, WHALE
from track_a.select import log, read_json, write_json, ev, flats_now, recent_engines, CHANNELS
from common.ws import load_params, PARAMS, load_states

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOGS = os.path.join(runtime_root(ROOT), "logs")
HUNT = dict(on=0,                # 1: this job owns params.books (track_a.select stopped); 0: report only, never writes params or state
            ...
            #                      실패하면 결정론으로 폴백한다. 판정은 며칠 뒤 `track_b.campaigns`: 불일치한 행에서 밀려난 쪽이 옳았나.
...
def exit_flags(r, held, hunt):
    ...
    # 없으므로 전량 청산까지 가지 않는다 — 승격 조건은 `track_b.campaigns` 에서 그 행의 전방 4h 꼬리가 markup(p10 −8.3%)보다 나쁠 때.
...
def ai_read(rows, hunt, log=log, held=()):
    ...
    (그 계산이 `track_b.campaigns`). **자격(통행료 게이트)은 건드리지 않는다** — vol·ATR·레버·현물·펀딩·twoway 는 측정 가능한
...
def pid_alive(path, match=("common.supervise select", "track_a.select")):
    """Why this job must not write params.books, or "" when it may. The pid must be alive AND its command line must be one of
    `match`: a Windows pid is reused, and `tasklist /FI "PID eq N"` only says somebody holds that number — track_a.select stopped
    2026-09-02 23:52, hunt wrote books all the next day, and a stranger inheriting its pid raised HUNT_BLOCKED at 09-04 00:16.
    Both forms are matched because logs/select.pid holds the SUPERVISOR's pid (`common.supervise select`, supervise.main writes its
    own), not the `track_a.select` child's. An unreadable command line blocks: refusing to write is the recoverable error, two
