...
stage 2 adds taker CVD / OI / funding series from the recordings and the fingerprint tables — NEXT 6.13).
  python -m track_b.whale SYM[,SYM...] [--end "YYYY-MM-DD HH:MM"] [--hours 48] [--step 1]
...
afterwards (the validation column, never an input). Pure functions `footprints` / `phase` are what track_b/hunt.py consumes.
...
THE MODEL (CONCEPT 실험 모드): an operator runs a coin through phases and each phase leaves footprints we record —
  markup    fresh money (24h volume >= episode_ratio x the coin's own 7-day median), a run into new highs, structure up or unreadable;
            shakeouts inside it are where the long cycle buys (the sweep-and-reclaim: track_a.sweeps).
...
from common.signal import wilder_atr, structure_side, zigzag_pivots
...
def timeline(sym, end_ms, hours=48, step=1, data=None, held=None, p=WHALE, fund_at=None, track_held=False):
    ...
    held coin — without both, a post-hoc timeline could never say squeeze or dead (audit 2026-09-03; track_b.phases fills them)."""
