"""선정기 반사실 — logs/hunt-history.jsonl 의 모든 스캔에서 "그때 그 코인에 들어갔다면?"을 걸어가며 세는 읽기 전용 도구.
    python -m track_b.campaigns [--stop live|N] [--hours 12] [--by off|ratio|run|atr|tw|age|spread|depth]
...
from track_b.hunt import flags_of, exit_flags, HUNT
...
def scans(path=None):
    out = []
    for line in open(path or os.path.join(runtime_root(ROOT), "logs", "hunt-history.jsonl"), encoding="utf-8"):
        d = json.loads(line)
