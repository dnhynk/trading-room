...
(엔진의 `pos["lots"]`와 같은 규칙). 로트가 다 팔리면 한 사이클로 확정한다 — RULES의 "사이클 = 로트가 다 팔린 횟수".
    ...
    python -m track_a.cycles [--from "2026-08-29 18:18"] [--side long|short] [--symbol SYM] [--top 20] [--csv PATH]
    ...
    for side in sorted({c["side"] for c in done}) + ["ALL"]:
        ...
        if side == "ALL":
            ...
            print(f"        이익 사이클 {sum(1 for n in net if n > 0):3d}개 {pos:+8.3f} / "
                  ...
                  f"  (수수료 {fee:.3f} 포함)")
    e = geometry_of(done)
    if e:      # 엔진 기하 — track_a/scan.py EDGE 의 출처(select 이 스캔 때마다 여기서 다시 잰다). 명목가 대비 %라 종목이 섞여도 더할 수 있다
        W, L, F, pw = e["win_pct"], e["loss_pct"], e["fee_pct"], e["p_win"]
        print(f"\n엔진 기하 (track_a/scan.py EDGE, n={e['n']}): win_pct {W:.3f}  loss_pct {L:.3f}  fee_pct {F:.4f}"
