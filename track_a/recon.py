"""백테스트 캘리브레이션 — 같은 시간·같은 사이즈로 돌린 백테스트와 실매매 장부를 대조한다.
    ...
    python -m track_a.recon [--day YYYYMMDD] [--min-hours 2] [--sym SYMBOL] [--quiet]
...
from track_a.backtest import run_files
from common.ws import load_params, portfolio
...
def tape_hours(day):
    ...
    out = {}
    for f in glob.glob(os.path.join(runtime_root(ROOT), "data", "ws", f"pub-{day}-*.jsonl*")):
