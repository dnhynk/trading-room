"""Basket manager for the engine.  python -m common.supervise select   (python -m track_a.select [--once] [--dry])
Every select.every_h hours (and at start) `track_a.scan.rank` scores the universe, logs/scan.json is rewritten (and every scan's rows
appended to logs/scan-history.jsonl), and `params.json["books"]` is brought toward the target basket. One engine per book
(common/supervise.py `cycle`).
...
THE BASKET (2026-09-02, "집중된 바구니"): `select.n` MAIN books plus one PROBE book.
  ...
  - `select.sigma_norm` = 1 scales the main shares by 1 / sigma_daily (the money cap then sits at the same distance in sigma on every
    book); off until the pair table (track_a.pair) shows the gross loss per cycle scaling with sigma (NEXT 8).
  ...
  Switching toward a single symbol is a decision, not a rule here: track_a.pair marks the estimator's #1 against the basket average every
...
Also written by this job: `strat.symbol` (the highest-ranked held symbol — the default for common.trade and for an unpinned engine),
...
The engine geometry the score charges (win / loss / fee per cycle) is re-measured from the live ledger at every scan (track_a.cycles
...
from track_a.scan import rank, MIN_TRIALS_H
from track_a.cycles import geometry, build
from common.ws import load_params, PARAMS, load_states, portfolio
...
def evidence(now, days, done=None, cuts=None):
    """Live evidence per symbol from the completed lot cycles closed in the last `days` (track_a.cycles) — a symbol's window starts at its
...
def apply(p, rows, wind, adds, flats, sel, recent=(), evict=(), probe=None, promote=None, probe_end=None, ev=None, st=None, now=None):
    ...
    held = list(books)
    if sp.get("symbol") not in held and held:                                     # the default symbol for common.trade and for an unpinned engine
