...
net of the toll — and disqualify the ones whose toll eats the edge. Report only; track_a/select.py owns the basket.
  python -m track_a.scan [--top 15] [--days 3] [--min-vol 50e6] [--n-books 4] [--json] [--sym TRUMPUSDT,ENAUSDT]
...
ranked on the one quantity our P&L barely contains:
  ...
  trials/h, outcome  a triple-barrier count (`trials`): every common.signal.candle_rule firing opens a trial resolved by +win_pct,
                     -loss_pct or the hold horizon. win_pct/loss_pct are the engine's own measured geometry (track_a.cycles summary),
...
HARD DISQUALIFICATION (flags; never a candidate, and a flagged holding is wound down — track_a/select.py): imp% >= fee% (our own
...
from common.bitget import Bitget, PRODUCT
from common.signal import STRAT, SIG, zigzag_pivots, wilder_atr, structure_side, candle_features, candle_rule
from common.ws import load_params, load_states, portfolio

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
WIN = 1440
# The engine's own geometry, measured from the live ledger (the track_a.cycles summary prints all three; re-read them after any change
...
def trials(c, sg, win, loss, hold_min, bounds):
    """Triple-barrier count of what the tape offers this engine, per window of closed 1m candles. Every common.signal.candle_rule firing
...
def flags_of(x, wins, net_total, min_vol=0.0, fee=0.0, min_trials_h=0.0):
    """Hard disqualification, never ranking: a flagged symbol is not a candidate and a flagged holding is wound down (track_a/select.py).
...
def rank(min_vol=5e7, days=3, syms=None, exclude=(), log=print, always=(), equity=None, n_books=1, edge=None, min_trials_h=0.0, corr_th=0.5):
    ...
    `always` = symbols measured even when they fail the volume gate (track_a/select.py passes the held books: a basket verdict needs
...
def main():
    ...
    from track_a.cycles import geometry
    ...
    if "--json" in sys.argv:
        ...
        with open(os.path.join(runtime_root(ROOT), "logs", "scan.json"), "w", encoding="utf-8") as f: json.dump(dict(t=time.strftime("%Y-%m-%d %H:%M:%S"), days=days, rows=rows), f)
