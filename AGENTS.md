...
C는 C3 `track_c.c3_runner`(`c3-rule-v3`)와 state v3 포트폴리오를 사용하며 **BTC만 실매매**한다. ETH·XRP·SOL은 기존 16종목과 함께 기록 전용이다. `bot/CONCEPT-C.md`가 계약, `track_c/README.md`가 운영, `track_c/AUDIT-C3-20260905.md`가 최신 수리 감사다. 공정가≥익절가·30/10초 모멘텀 veto/브레이크·32초 매도흐름 조건과 1만원·3틱 손절은 유지한다. 컨셉·과거 통계 주장도 감사 대상이며 수익 우위는 미입증이다. `track_c/evaluation-c3.json`은 BTC 단독 재개 **2026-09-06 00:06:30.977KST부터10×24시간**, 알림 기준선도 같은 시각이며 이전 4종목 평가는 별도 보존한다. Slack 손익·캠페인·승률은 BTC만 집계하고 종목 필터는 자정 이후에도 유지한다. 원본 장부·실제 자본·위험 예산은 초기화하지 않는다. `python -m track_c.deploy_c3 status`로 mode·PAUSE·모멘텀·연결·캠페인·잔량을 확인한다. C2 worker는 폐기됐고 재기동하지 않는다. 진입 알림OFF/청산체결만 발송하며 매수 회계는 유지한다. C 배포는 `c3_upgrade`의 flat 절차를 따른다. 기본 v3 프로필은 설정5개만, 이번 `prepare --profile btc-only`는 `coins`·`record_coins`만 교체하고 나머지를 보존한다. 아래 A/B 규칙은 A/B에만 해당한다.
