...
## 현재 트랙 C 운영 (2026-09-06)

C는 C3 `track_c.c3_runner`(`c3-rule-v4`)와 state v3 포트폴리오를 사용한다. **BTC만 실매매·진입 공정가 편차 하한2틱·기준금액2만원**이다. 진입 시300초 과거 변동성으로 stop/stop_limit을 계산해 고정하고, 목표 수량이 작으면 최소 청산금액105%를 충족하도록 올리되 기준 수량·현금·총위험 한도를 지킨다. 보유 가치 청산은 완료된 과거 경로20개가 준비된 경우만 적용하며 기존 익절1틱·모멘텀/방어/시간 보호를 유지한다. ETH·XRP·SOL 및 기존16종목은 기록 전용이다. 계약은 `bot/CONCEPT-C.md`, 운영·수리는 `track_c/REPAIR-C3-20260906.md`, 계산 설계는 `track_c/EXIT-C3-20260906.md`다. release `20260906-024813-803e30a1e473`, **재개/전향 평가 2026-09-06T02:59:02.270+09:00부터10×24시간**, 소스36개 등록. Slack은 기존00:06:30.977KST부터 BTC·진입편차2틱 이상을 집계하며 이 필터는 자정에도 유지한다. 원본 장부·실제 자본·위험 누적은 초기화하지 않는다. 알림 release `20260906-031638-9f5d6c3ad7a6`, 진입OFF·청산체결만·손익 원화 소수점 첫째 자리. 이월 재고는 원래 진입별로 추적해 실제 후속 매도 때 완료하며 총 진입/완료/진행/잔량 대기·승패를 함께 표시한다. 같은 매도의 FILL/CLOSE 알림은 합친다. `c3_upgrade prepare --profile execution → switch → resume`의 flat 절차를 사용한다. 소유 PAUSE 상태에서 거래를 재개하지 않은 교체본만 `--continue-paused`로 수리할 수 있다. 20초 주 루프 watchdog·종료 후 독점 복구·연결 풀을 사용하며 가격 표본 작업은 정상 REST 대기 중 계속된다. 루트32GiB/public16GiB/여유1GiB 보호선. 수익 우위와 거래 용량은 미입증이다. A/B·C2·독립 leaders worker는 재개하지 않는다. 실제 상태는 `python -m track_c.deploy_c3 status`로 확인한다.

`bot/TRACKS.json`과 `bot/BOOT.md`의 현재 운영 상태를 먼저 읽는다. A/B는 사용자 지시로 일시정지했고 아래 A/B 감시견·Monitor를 자동 재개하지 않는다. C는 AWS `trading-room-c.service`로 실거래하며 운용자본은 계좌 잔액 전체 복리다. C 계약은 `bot/CONCEPT-C.md`다. 알림은 독립 `trading-room-c-notify.service`가 `bot.notify`의 C 원화 경로로 보낸다. **최신 사용자 지시: 체결·캠페인·누적·미실현 손익은 원화 소수점 첫째 자리까지 표시한다.** 내부 계산의 정밀도는 보존한다. C `data/ledger.sqlite`/`status.json`만 읽으며 A/B USDT 계기판을 섞지 않는다. `python -m track_c.deploy_notify status`로 기존 worker를 확인하고 중복 릴레이를 시작하지 않는다. C 알림의 당일은 KST이며 엔진의 UTC 일일 위험 제한과 구분한다. C 알림 작업 때문에 매매 엔진을 재기동하지 않는다. 상세 운영·장애 확인은 `track_c/README.md`다.
