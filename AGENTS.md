...
C는 C3 `track_c.c3_runner`(`c3-rule-v4`)와 state v3 포트폴리오를 사용한다. **BTC만 실매매·진입 공정가 편차 하한2틱·기준금액2만원**이다. 진입 시300초 과거 변동성으로 stop/stop_limit을 계산해 고정하고, 목표 수량이 작으면 최소 청산금액105%를 충족하도록 올리되 기준 수량·현금·총위험 한도를 지킨다. 보유 가치 청산은 완료된 과거 경로20개가 준비된 경우만 적용하며 기존 익절1틱·모멘텀/방어/시간 보호를 유지한다. ETH·XRP·SOL 및 기존16종목은 기록 전용이다. 계약은 `bot/CONCEPT-C.md`, 운영·수리는 `track_c/REPAIR-C3-20260906.md`, 계산 설계는 `track_c/EXIT-C3-20260906.md`다. release `20260906-024813-803e30a1e473`, **재개/전향 평가 2026-09-06T02:59:02.270+09:00부터10×24시간**, 소스36개 등록. Slack은 기존00:06:30.977KST부터 BTC·진입편차2틱 이상을 집계하며 이 필터는 자정에도 유지한다. 원본 장부·실제 자본·위험 누적은 초기화하지 않는다. 알림 release `20260906-024021-6ff74fb7c709`, 진입OFF·청산체결만·손익 원화 소수점 첫째 자리. `c3_upgrade prepare --profile execution → switch → resume`의 flat 절차를 사용한다. 소유 PAUSE 상태에서 거래를 재개하지 않은 교체본만 `--continue-paused`로 수리할 수 있다. 20초 주 루프 watchdog·종료 후 독점 복구·연결 풀을 사용하며 가격 표본 작업은 정상 REST 대기 중 계속된다. 루트32GiB/public16GiB/여유1GiB 보호선. 수익 우위와 거래 용량은 미입증이다. A/B·C2·독립 leaders worker는 재개하지 않는다. 실제 상태는 `python -m track_c.deploy_c3 status`로 확인한다.
