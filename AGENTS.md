...
C는 C3 `track_c.c3_runner`(`c3-rule-v3`)와 state v3 포트폴리오를 사용하며 **BTC만 실매매, 신규 주문 공정가 편차 하한2틱**이다. ETH·XRP·SOL은 기존16종목과 함께 기록 전용이다. `bot/CONCEPT-C.md`가 계약, `track_c/README.md`가 운영, `track_c/AUDIT-C3-20260905.md`가 수리 감사다. 2틱은 사용자 지정 기본값이고 수익 최적값은 아니다. 공정가≥익절가·30/10초 모멘텀 veto/브레이크·32초 매도흐름·1만원·3틱 손절을 유지한다. release `20260906-005506-272bfcbfc659`, 하한2틱 재개/평가는 **2026-09-06 00:57:27.869KST부터10×24시간**이다. 후속 지시로 Slack은 기존 BTC 시작 **00:06:30.977KST부터 진입 주문 편차2틱 이상**만 손익·수익률 분자·캠페인·승률에 집계한다. `notifications/baseline.json`의 `coins`·`entry_dev_min_ticks` 필터는 자정에도 유지한다. 평가와 표시 창을 혼동하거나 원본 장부·자본·위험 예산을 초기화하지 않는다. 알림 release `20260906-005913-53649994640e`, 진입OFF/청산체결만 발송이다. 무손절 다중 유닛 보유는 `track_c/PROPOSAL-C3-INVENTORY-20260906.md`의 검토 제안이며 live 청산 해제는 적용하지 않았다. `python -m track_c.deploy_c3 status`로 실제 상태를 확인한다. C 배포는 `c3_upgrade`의 flat 절차를 따르며 기본 v3 프로필은 설정5개, `btc-only`는 종목2개, `entry-2`는 `entry_ticks`만 교체한다. A/B·C2 worker는 재개하지 않는다. 아래 A/B 규칙은 A/B에만 해당한다.
