...
- **수정 세션**(사용자가 "수정 세션" / "메커니즘 바꾸자"라고 열 때): 메모리 `cycle-harness-plan` → `bot/CONCEPT.md` → `bot/RULES.md`를 읽고 아래 "엔진 변경 절차"대로 일한다. 끝나면 메모리·RULES를 현재 계약으로 갱신하고 한 줄 보고 후 종료한다.
...
- 엔진은 `params.json`의 (symbol, side)를 배타 소유한다 — 현재 `TRUMPUSDT` long. 엔진 주문은 clientOid `cycL-`/`cycS-`, 엔진 스탑은 그 방향의 pos_loss 플랜. 그 밖의 포지션·주문·플랜은 사용자 것이며 절대 건드리지 않는다.
...
## 엔진 변경 절차 (수정 세션)
CONCEPT/RULES 문장 먼저 → `bot/signal.py`·`bot/cycle.py` 최소 diff → `python -m unittest bot.test_signal bot.test_cycle` → `python -m bot.backtest data/ws/pub-20260829-0[4-6].jsonl.gz`(기준 테이프; 현재 수치는 RULES.md) → `-m bot.cycle` 자식만 Stop-Process(감시견이 5초 뒤 올림; 포지션·스탑·사이즈는 state.json에 남음) → `python -m bot.preflight`. 코드 변경 없는 재기동 금지. `mode`·심볼·`sides`·쌍검 변경은 사용자 승인 후 포지션 0에서만(live 엔진은 플랫이 될 때까지 스스로 보류한다). 숫자는 전부 휴리스틱이고 근거는 야간 리포트·replay·tune(리포트 우선, `--apply`는 승인 후).

## 운영 메모 (Windows)
- 감시견 4개는 PowerShell `Start-Process -WindowStyle Hidden`으로 분리 실행: `python -m bot.supervise record|cycle|nightly|sweep`. pid는 `logs/<job>.pid`, 로그는 `logs/<job>.log`.
