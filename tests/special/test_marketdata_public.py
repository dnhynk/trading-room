...
class MarketDataTests(unittest.TestCase):
    ...
    def test_snapshot_receipt_isolates_failed_endpoint_and_keeps_benchmarks(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            self.assertTrue(receipt.paths["spot_candles_market_4H"].exists())
            self.assertTrue(receipt.paths["futures_candles_mark_1H"].exists())
            self.assertTrue(receipt.paths["futures_candles_index_5m"].exists())
            self.assertEqual(27, len(receipt.counts))
