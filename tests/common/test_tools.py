"""Invariants of the evidence tools (track_a/replay.py hold-vs-sell, track_a/capture.py direction capture).  python -m unittest tests.common.test_tools"""
import os, unittest
from track_a.replay import hold_pnl
from track_a.capture import capture, held_at
...
class Sweeps(unittest.TestCase):
    def test_a_dip_under_a_confirmed_pivot_that_reclaims_is_one_sweep_with_its_depth(self):
        from track_a.sweeps import sweeps
        secs = []
...
class Follow(unittest.TestCase):
    def test_the_active_side_follows_the_hint_and_flips_only_when_flat(self):
        from track_a.backtest import Engine
...
class SymbolAttribution(unittest.TestCase):
    ...
    def setUp(self):
        import tempfile, os
        from track_a import capture
        from track_a import recon
    ...
    def test_the_capture_timeline_takes_only_its_own_symbol_and_side(self):
        from track_a.capture import timeline
    ...
    def test_recon_sizes_and_qstep_come_from_that_symbols_own_events(self):
        from track_a.recon import load_events, sizes, qstep_of, live
...
class Ledger(unittest.TestCase):
    """track_a.cycles reads the engine's lot bookkeeping back from FILL events: LIFO, except a de-risk cut the engine booked against the core
    ...
    def test_a_core_cut_reduces_the_first_lot_and_the_unit_cycles_whole(self):
        from track_a.cycles import build, geometry_of, geometry
...
class LedgerWindow(unittest.TestCase):
    """track_a.cycles.build(until=...): a report for a past day must not read the cycles that came after it (track_b.phases' ledger attributed
...
class Slippage(unittest.TestCase):
    """track_a.slip joins each fill to the mid its order saw at arrival (PLACE / TAKER .mid) by clientOid; cost is signed by the trade's direction."""
    LINES = [
...
class Pairing(unittest.TestCase):
    """track_a.pair: the day's scan medians per symbol beside the live cycles closed that day."""
    def test_estimator_and_live_join_on_symbol_and_utc_day(self):
        import tempfile, os, json
        from track_a import pair
...
class BacktestSizing(unittest.TestCase):
    def test_equity_freezes_the_live_sizes_and_switches_the_fractions_off(self):
        from track_a.backtest import size_from_equity
...
class SupervisorJob(unittest.TestCase):
    """common.supervise puts every child in a Windows job object that dies with the supervisor (2026-09-03: a recorder orphaned by a
...
class SelectorCounterfactual(unittest.TestCase):
    """track_b/campaigns.py walks an entry forward and asks which came first, the stop or an exit. p from this decides the sign of
    ...
    def test_the_stop_is_read_before_any_exit_on_the_same_scan(self):
        from track_b.campaigns import campaign
    ...
    def test_the_stop_distance_follows_the_live_geometry_per_entry_row(self):
        ...
        from track_b.campaigns import stop_pct
...
class NightlySymbols(unittest.TestCase):
    ...
    def test_engines_seen_that_day_join_the_books_and_a_gone_book_keeps_its_side(self):
        import time
        from common.nightly import symbols
...
class MultiBook(unittest.TestCase):
    ...
    def test_the_pool_admits_cap_books_holds_while_positioned_or_armed_and_refuses_on_the_days_loss(self):
        ...
        from track_a.backtest import SimPool
        class B:
    ...
    def test_books_are_merged_on_one_clock_and_a_tie_goes_to_the_earlier_book(self):
        from track_a.backtest import _merge, _hour_key
