"""Invariants of the pump-coin hunting side pipeline (track_b/hunt.py): phase names the side, the toll vetoes, a book per confirmed
eligible coin under the capital pool (one book without it), phase exits rotate without cooldown, episode deaths cool down.  python -m unittest tests.b.test_hunt"""
import os, shutil, tempfile, types, unittest
from track_b import hunt
from track_b.hunt import flags_of, exit_flags, verdict, apply, pid_alive, HUNT
...
class ScanClock(unittest.TestCase):
    ...
    def test_the_next_scan_is_the_next_grid_slot_after_now(self):
        from track_b.hunt import next_scan_t, SCAN_OFFSET_S
...
class TheOtherWriter(unittest.TestCase):
    ...
    def test_a_reused_pid_running_something_else_does_not_block(self):
        self.pid("14976")                   # the pid is alive, but it is not track_a.select: the 2026-09-04 00:16 HUNT_BLOCKED
        self.cmdline('"C:\\Python313\\python.exe" -m common.supervise hunt\n')
    ...
    def test_the_real_other_writer_blocks_in_both_of_its_forms(self):
        self.pid("4242")
        self.cmdline('"C:\\Python313\\python.exe" -m common.supervise select\n')       # what select.pid actually holds: the supervisor
        self.assertIn("common.supervise select", pid_alive(self.pidfile))
        self.cmdline('"C:\\Python313\\python.exe" -u -m track_a.select\n')              # the child it spawns
        self.assertIn("track_a.select", pid_alive(self.pidfile))
