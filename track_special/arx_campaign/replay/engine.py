...
def replay(bars: Iterable[ReplayBar], quantity: Decimal, fee_rate: Decimal=Decimal("0.0006"), stop: Decimal|None=None, take_profit: Decimal|None=None) -> ReplayResult:
    ...
    first=bars[0]; q=min(quantity, first.available_quantity)
    ...
    for bar in bars:
        funding += q*bar.open*max(bar.funding_rate,Z)
        ...
        if bar.maintenance_rate > Z and adverse_bid <= entry*bar.maintenance_rate: exit_price=adverse_bid; notes.append("synthetic_liquidation"); break
    ...
    return ReplayResult(q,quantity-q,q*entry,q*exit_price,fees,funding,net,"synthetic_liquidation" in notes,tuple(notes))
