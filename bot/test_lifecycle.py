...
from bot.lifecycle import require_active, startup_block_reason
...
class LifecycleTests(unittest.TestCase):
    ...
    def write(self, hunter=1):
        (self.root / "bot" / "TRACKS.json").write_text(json.dumps(self.state), encoding="utf-8")
        (self.root / "params.json").write_text(json.dumps({"hunt": {"on": hunter}}), encoding="utf-8")
    ...
    def test_missing_or_malformed_control_never_enables_trading(self):
        state = self.root / "bot" / "TRACKS.json"
