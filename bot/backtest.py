...
equity-scaled sizing is not modelled (no equity offline) — fixed unit_qty / cap_usdt from params apply.
...
from bot.signal import Features, Strategy, STRAT, apply_fill, pos_stats, book_params, sim_match
...
class Book:
    ...
    def __init__(self, feat, strat, side, n_sides=1, qstep=0.1):
        bp = book_params(strat or {}, side, 0.001, n_sides, qstep)
...
class Engine:
    ...
    # -- fills
    def on_fill(self, bk, role, qty, px, fee, oid):
        n0 = len(bk.pos["lots"])
        pnl = apply_fill(bk.pos, bk.s, role == "buy", qty, px, oid=oid, fee=fee); bk.realized += pnl; bk.day_realized += pnl; bk.strat.on_fill(role, qty)
        ...
        for role in ("buy", "trim"):
            ...
            if want is not None and role == "trim" and want[2] == "taker":
                ...
                if t - bk.taker_t >= 5:
                    ...
                    self.on_fill(bk, "trim", want[1], px, want[1] * px * TAKER, f"{OID}{bk.side[0]}m{bk.seq}")
                continue
            ...
            if want is not None and w is None:
                ...
                bk.work[role] = dict(oid=f"{OID}{bk.side[0]}{role[0]}{bk.seq}", px=want[0], qty=want[1], filled=0.0, queue=lvl.get(want[0], 0.0), t=t); bk.replaced[role] = t
        qty, _ = pos_stats(bk.pos)
    ...
    def sim_trades(self, trades):
        ...
        for px, size, side in trades:
            ...
            for (bk, role), w, fill in sim_match(orders, px, size, side, self.qstep):
                self.on_fill(bk, role, fill, w["px"], fill * w["px"] * MAKER, w["oid"])
