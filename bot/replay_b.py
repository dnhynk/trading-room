...
python -m bot.replay_b --scorecard logs/research-b-20260905.json --output logs/replay-b-20260905.json
...
from bot import backtest, signal
...
def panel(campaigns):
    ...
    for (sym, side), cs in sorted(groups.items()):
        ...
        files = []
        while start <= end:
            name = os.path.join(ROOT, "data", "ws", start.strftime("pub-%Y%m%d-%H.jsonl.gz"))
...
def reference(revision):
    ...
    modules = []
    old_signal = sys.modules["bot.signal"]
    try:
        for name in ("signal", "backtest"):
            source = subprocess.check_output(["git", "show", f"{revision}:bot/{name}.py"], cwd=ROOT).decode("utf-8")
            module = types.ModuleType(f"bot.{name}")
            module.__file__ = os.path.join(ROOT, "bot", f"{name}.py")
            exec(compile(source, module.__file__, "exec"), module.__dict__)
            modules.append(module)
            if name == "signal": sys.modules["bot.signal"] = module
        return modules
    finally: sys.modules["bot.signal"] = old_signal
...
def main():
    ...
    for name in ("signal", "backtest", "risk", "cycle", "hunt"):
        with open(os.path.join(ROOT, "bot", name + ".py"), "rb") as fh: hashes[name] = hashlib.sha256(fh.read()).hexdigest()
    ...
    for name, module, execution in variants:
        ...
        with patch.object(module, "load_params", return_value=p), patch.object(module, "contract_meta", side_effect=cache.contract), \
                ...
                patch.dict(sys.modules, {"bot.signal": old_signal if name == "reference" else signal}):
