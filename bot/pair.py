...
Live side: bot.cycles — completed lot cycles that day: n, cycles per engine-hour (hours = span from the day's first cycle open to its
...
NEXT 6.1) and cyc/h / tr/h (how many trials the engine converts; NEXT 6.2 — the product form assumes a constant).
  python -m bot.pair [--day YYYYMMDD] [--sym SYMBOL]
Read-only: logs/scan-history.jsonl, logs/events.jsonl."""
import json, os, sys, time
from collections import defaultdict

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
HIST = os.path.join(ROOT, "logs", "scan-history.jsonl")
...
def live(done, day=None, sym=None):
    """{(symbol, day): {n, hours, cyc_h, win, gwin, gloss}} from bot.cycles' completed cycles (closed that UTC day)."""
    g = defaultdict(list)
    for c in done:
        s = c.get("symbol")
    ...
    for k, cs in g.items():
        ...
        gr = [(c["gross"] / (c["qty"] * c["entry"]) * 100, c["net"]) for c in cs if c["qty"] and c["entry"]]
        wins = [a for a, n in gr if n > 0]; losses = [a for a, n in gr if n <= 0]
...
def table(est, lv):
    ...
    lines = [f"  {'symbol':<10}{'day':<9}{'scans':>6}{'p_up':>6}{'tr/h':>6}{'edge':>7}{'imp%':>7}{'entry':>6} | {'n':>4}{'hours':>6}{'cyc/h':>6}{'win':>6}{'gwin%':>7}{'gloss%':>7} | {'win-p_up':>9}{'cyc/tr':>7}"]
    for k in sorted(set(est) | set(lv)):
        ...
        lines.append(f"  {k[0]:<10}{k[1]:<9}{e.get('scans', 0):>6}{f(e.get('p_up'), 6)}{f(e.get('trials_h'), 6)}{f(e.get('edge'), 7, 3)}{f(e.get('impact'), 7, 3)}{f(e.get('entry'), 6, 1)} | "
                     f"{l.get('n', 0):>4}{f(l.get('hours'), 6, 1)}{f(l.get('cyc_h'), 6)}{f(l.get('win'), 6)}{f(l.get('gwin'), 7, 3)}{f(l.get('gloss'), 7, 3)} | {f(dw, 9)}{f(ratio, 7)}")
    return "\n".join(lines)

def main(argv):
    from bot.cycles import build
