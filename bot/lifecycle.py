...
def startup_block_reason(job, root=ROOT):
    try:
        status = json.loads((Path(root) / "bot" / "TRACKS.json").read_text(encoding="utf-8-sig"))
        ...
        if job == "hunt":
            ...
            params = json.loads((Path(root) / "params.json").read_text(encoding="utf-8-sig"))
        ...
        if status["tracks"][track]["status"] != "active":
            return f"track {track} is not active; see bot/SUSPENDED-20260905.md"
