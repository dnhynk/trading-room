...
from .coinone import CoinoneError, CoinoneReadOnly, Credentials
...
from .runner import entry_signal_current
...
class InfrastructureTests(unittest.TestCase):
    def test_actual_lowercase_tickers_join_uppercase_market_contracts(self):
        ...
        from .coinone import symbol
        self.assertEqual(symbol('W'), 'W')

    def test_account_wait_cannot_revive_expired_signal_with_a_fresh_book(self):
        from types import SimpleNamespace
        market = SimpleNamespace(fresh=lambda now: True, features=SimpleNamespace(bid=100, f=dict(t=11, bs10=.7, v=.1)))
        sig = dict(t=10, mid=100)
        self.assertTrue(entry_signal_current(market, sig, 11010, 12000, 1500))
        market.features.f["t"] = 13
        self.assertFalse(entry_signal_current(market, sig, 11010, 14000, 1500))
        sig["t"] = 13
        market.features.f["bs10"] = .4
        self.assertFalse(entry_signal_current(market, sig, 14010, 14100, 1500))
        market.features.f.update(bs10=.7, v=-1.1)
        self.assertFalse(entry_signal_current(market, sig, 14010, 14100, 1500))
    ...
    def test_remote_bundle_does_not_contain_keys_or_a_b_runtime(self):
        import tarfile, io
        raw, digest, manifest = bundle()
        with tarfile.open(fileobj=io.BytesIO(raw), mode="r:gz") as tar:
            names = tar.getnames()
        self.assertTrue(names)
        self.assertFalse(any(".env" in n or n.endswith(".pem") or n == "bot/cycle.py" for n in names))
        self.assertEqual(set(names)-{"manifest.json"}, set(manifest))
