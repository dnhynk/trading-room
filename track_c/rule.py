...
def assess(cfg, *, coin, bid, ask, tick, dev, contract, units, cash, risk_remaining):
    ...
    stop = price_floor(units, entry - D(str(tick)) * int(cfg['stop_ticks']))
    stop_limit = price_floor(units, stop - price_unit(units, stop))
    ...
    plan = dict(reason=None, qty=str(qty), entry=str(entry), stop=str(stop), stop_limit=str(stop_limit), take_profit=str(take),
                ...
                target_ticks=int(cfg['target_ticks']), dev_ticks=dev, research=True, score=dev, expected_net_bp=None, p_fill=None)
    return dict(common, accepted=True, reason='rule', plan=plan, best=dict(score=dev))


def hold(cfg, *, dev, bid, entry, tick, age_s):
    """Holding decision after a fill. Unknown fair value keeps stop/time protection only."""
    if bid is not None and bid <= entry - float(tick) * int(cfg['stop_ticks']) + 1e-12:
        return dict(hold=False, reason='stop')
    if dev is not None and dev < float(cfg['defend_ticks']):
...
def cancel_entry(cfg, *, dev):
    ...
    return dev is None or dev < float(cfg['cancel_ticks'])
