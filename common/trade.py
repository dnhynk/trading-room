"""My hands. Usage:
  python -m common.trade status
  python -m common.trade open long|short SIZE [--sl P] [--tp P]     # market entry carrying the SL/TP as presets (protected from the fill), then position SL/TP
  python -m common.trade add  long|short SIZE [--sl P] [--tp P]     # add to existing position (position SL/TP follow the size; --sl/--tp move them)
  python -m common.trade tpsl long|short [--sl P] [--tp P]          # move position SL/TP in place (modify), placing only what is missing; nothing is cancelled first
  python -m common.trade close long|short [SIZE]                    # market reduce (whole position if SIZE omitted)
  python -m common.trade limit long|short SIZE PRICE [--close]      # post-only maker order (entry, or --close to reduce)
  python -m common.trade tp long|short PRICE                        # replace TP with post-only limit close of whole position
  python -m common.trade cancel all|ORDER_ID                        # cancel pending limit orders
...
from common.bitget import from_env, BitgetError
from common.ws import load_params, load_states, portfolio, strat_for

ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
LOG = os.path.join(runtime_root(ROOT), "logs", "trades.jsonl")
