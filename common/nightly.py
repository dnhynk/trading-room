"""Nightly evidence report, run under the supervisor (python -m common.supervise nightly): waits for 00:10 UTC each day, then writes
...
and the side-automation counterfactuals; once per basket: track_a.capture, (2) track_a.recon — what the backtest got wrong against the live ledger
...
--apply), (4) the symbol scanner. python -m common.nightly --now runs it once immediately for the previous day (or --day YYYYMMDD)."""
...
def report(day):
    allf = sorted(f for f in glob.glob(os.path.join(runtime_root(ROOT), "data", "ws", "pub-*.jsonl*")) if not (f.endswith(".jsonl") and os.path.exists(f + ".gz")))
    ...
    if files:
        for sym in syms:                                # every book gets its own tables: a basket's evidence is per symbol, never one default symbol's
            ...
            bt = ["track_a.backtest"] + files + ["--sym", sym] + gone
            out.append(f"\n# {sym}\n")
            out.append("## signals (replay, forward 15m; split by source / volume decay / CVD divergence / value area / regime)\n")
            out.append(run_cmd(["track_a.replay"] + warm + files + ["--sym", sym, "--quiet", "--day", day, "--by", "sell_decay,buy_decay,cvd_div,cvd_div_bear,vp_va,vp_dens,rg_er,leg_ow,U,D,side_hint_1h,side_hint_15m,daily_trend,dbl,brk"]))
            out.append("\n## legs (where the deceleration detectors fire vs the real extremes; speed-model evidence)\n")
            out.append(run_cmd(["track_a.legs"] + warm + files + ["--sym", sym, "--quiet", "--day", day]))
            ...
            out.append(run_cmd(["track_a.sweeps"] + warm + files + ["--sym", sym, "--quiet", "--day", day]))
            out.append("\n## phases (does the lifecycle read pay: forward move / live cycles per phase / order-flow footprints; 세력대항마 stage 2, NEXT 6.13)\n")
            out.append(run_cmd(["track_b.phases", sym, "--day", day] + files, timeout=1800))
        out.append("\n# basket\n")
        ...
        out.append(run_cmd(["track_a.capture", "--day", day]))
        out.append("\n## recon (백테스트 캘리브레이션: 경계 이벤트 없는 구간만 골라 같은 사이즈로 돌린 백테스트와 실매매 장부를 대조 — 아래 backtest 기반 수치는 전부 이 오차를 진다)\n")
        out.append(run_cmd(["track_a.recon", "--day", day], timeout=3600))   # books 의 심볼마다 구간별 백테스트를 돌린다
