"""순환매 harness.  python -m common.cycle   (kept alive by: python -m common.supervise cycle)
params.json (hot-reloaded every second): "strat" = symbol, sides (["long"], ["short"] or ["long","short"] = 쌍검술; falls back to
"side"), mode dry|live, sizes, steps, cap, daily_loss_limit, adopt; "sig" = signal thresholds. Key meanings: common/signal.py.
...
One feature stream (common/signal.py Features) feeds one Book per side; each Book has its own Strategy, position, orders, stop, daily
...
stops stay), RESUME (clears HALTs; deleted once applied). `books[sym].wind_down` is the same as PAUSE for one symbol — track_a/select.py
...
from common.signal import Features, Strategy, STRAT, SIG, apply_fill, pos_stats, book_params, sim_match, sim_book, unit_under_cap
from common.ws import WS, PUB_URL, PRV_URL, PRIVATE_ARGS, INST, load_params, PARAMS, strat_for, portfolio, outside_books, load_states
...
class Book:
    ...
    async def on_fill(self, role, qty, px, fee, oid, scope="maker", lot=None):
        ...
        self.ev("FILL", role=role, qty=qty, px=px, fee=rnd(fee), pnl=rnd(pnl), scope=scope, pos_qty=rnd(q), avg=rnd(avg), realized=rnd(self.realized), oid=oid,
                lot="core" if lot == 0 else None,   # a de-risk cut under units reduces the core lot, not the LIFO unit (track_a.cycles follows this)
