"""Bitget v2 WebSocket client + raw recorder.
  python -m common.ws record           # record public channels from params.json["record"] (hot-reloaded) + private channels
  python -m common.ws probe [SYMBOL]   # 20s check: prints login/subscribe acks and the first message of every channel
...
DATA = os.path.join(runtime_root(ROOT), "data", "ws")
PARAMS = os.path.join(ROOT, "config", "bitget.json")
...
def strat_for(params, symbol=None):
    ...
    from common.signal import STRAT
...
def load_states(strict=False):
    ...
    out = {}
    for p in glob.glob(os.path.join(runtime_root(ROOT), "logs", "state-*.json")):
        try:
    ...
    if not out:
        try:
            with open(os.path.join(runtime_root(ROOT), "logs", "state.json"), encoding="utf-8") as f: st = json.load(f)
            if st.get("symbol"): out[st["symbol"]] = st
