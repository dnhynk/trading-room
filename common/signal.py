...
SIG = dict(vol_hl=300, v_hl=8, a_lag=5, swing_s=600, dip_min_atr=3.0, v_fast=1.0, v_slow=0.3, hold_s=3,
           ...
           # 1m-candle rule = the manual watcher's speed_line (common/watch.py v13, 2026-08-29); emitted with src="1m"
...
class Features:
    ...
    def _candle_closed(self):
        ...
        if self.p["c1_on"]: self.pending += candle_rule(cl, cf, self.vr_hist, self.p)   # the manual watcher's rule, shared with track_a/scan.py's engine proxy
